...
class JSONDataSharingTests(TestCase):
    ...
    def test_public_home_page_is_available_without_login(self):
        ...
        self.assertContains(response, "Explore the platform")
        self.assertNotContains(response, f'href="{reverse("search")}"')
        self.assertNotContains(response, f'href="{reverse("upload_json")}"')
        ...
        self.assertContains(response, "Synthetic records illustrate")
    ...
    def test_signed_in_user_can_return_home_without_losing_session(self):
        ...
        self.assertContains(response, "The material record behind every simulation")
        self.assertContains(response, "Enter workspace", count=1)
        self.assertContains(response, f'href="{reverse("search")}"', count=1)
        self.assertContains(response, "Explore the platform")
        for name in ("upload_json", "json_data_list", "charts", "share", "account_settings", "password_change"):
